...
namespace CustomUI{
    public class TopUI : MonoBehaviour
    {
        ...
        /// <param name="isSave">다시 보여줄 안내 메세지 인가?</param>
        /// <param name="time"></param>
        public void ShowNotice(string _note,bool isSave, float time = 5.0f, float delay = 0.0f)
        {
            ...
            NoticeManager.Instance.AddNotice(_note);
            if (isSave) { note = _note;}
            
            m_ShowNoticeCroutine = ShowNoticeCroutine(_note, time, delay);
            StartCoroutine(m_ShowNoticeCroutine);
        }

        /// <summary> 저장된 안내 팝업을 나타냅니다. </summary>
        /// <param name="time"></param>
        //public void ShowSaveNotice(float time = 5.0f)
        //{
        //    if (!notice_group.gameObject.activeSelf) notice_group.gameObject.SetActive(true);
        ...
        IEnumerator ShowNoticeCroutine(string _note, float time, float delay)//일정 시간이 지난 후 투명하게 조절한다.
        {
        ...
        {
            //UI.semiStaticUI.ShowNoticeList();
        }
