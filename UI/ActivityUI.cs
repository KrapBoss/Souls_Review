...
namespace CustomUI
{
    public class ActivityUI : MonoBehaviour
    {
        ...
        public void AroundMonster(float ratio)
        {
            ...
            {
                Color c = aroundMonster_Image.color;
                c.a = 0;
                aroundMonster_Image.color = c;
            }
            else
        ...
        IEnumerator cor_ShowItemText = null;
        /// <summary> 아이템에 대한 정보를 표기합니다. </summary>
        /// <param name="itemName"></param>
        public void SetItemText(string itemName)
        {
            txt_getItem.text = itemName;

            if(cor_ShowItemText != null) StopCoroutine(cor_ShowItemText);
            cor_ShowItemText = GetItemTextCoroutine();
            StartCoroutine(cor_ShowItemText);
        }
        IEnumerator GetItemTextCoroutine()
        {
            txt_getItem.enabled = true;
            float t = 0;
            float time = 2.0f;
            while (t < time)
            {
                Color color = txt_getItem.color;
                color.a = Mathf.Lerp(1.0f, 0.0f, t / time);
                txt_getItem.color = color;
