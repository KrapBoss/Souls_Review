...
public class LibraryEvent : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isActivation)
        {
            ...
            float _duration = Duration * (GameConfig.IsPc() ? 1 : 2.0f);
            float ratio = (applyDuration / _duration) + (applyDuration / _duration);
            float progress = (applyDuration / _duration);
            UpdateTimer();

            source.pitch = 1.0f + ratio;
    ...
    IEnumerator OpenBookShelfCroutine()
    {
        ...
        //UI.topUI.ShowNotice(LocalLanguageSetting.Instance.GetLocalText("Tip", "Shift"), false);

        yield return new WaitForSeconds(1.0f);
        EventManager.instance.ActiveSouls(GhostSpawnPosition.position);

        VoiceLineManager.Instance.EnterLine(new() { clip = "C_libraryClear1", delay = 0, line = "C_libraryClear1", reverb = true });
        VoiceLineManager.Instance.EnterLine(new() { clip = "C_libraryClear2", delay = 0, line = "C_libraryClear2", reverb = true });
    ...
    public void EventStart()
    {
        ...
        VoiceLineManager.Instance.EnterLine(new() { clip = $"C_libraryStart{deadCount}", delay = 0, line = $"C_libraryStart{deadCount}", reverb = true });
        VoiceLineManager.Instance.ShowLine();
    } 
    ...
    IEnumerator GameOverCroutine()
    {
        if (isActivation)
        {
            ...
            */
            ...
            //    VoiceLineManager.Instance.EnterLine(new() { clip = $"C_libraryDead{deadCount}", delay = 0, line = $"C_libraryDead{deadCount}", reverb = true });
            //    VoiceLineManager.Instance.ShowLine(); 
            //}
            //else
            //{
            ...
            Debug.Log($"도서관 죽음 {deadCount}");
            VoiceLineManager.Instance.EnterLine(new() { clip = $"C_libraryDead{deadCount}", delay = 0, line = $"C_libraryDead{deadCount}", reverb = true });
            VoiceLineManager.Instance.ShowLine();


            //삐이 소리
            AudioManager.instance.PlayEffectiveSound("Tinnitus", 0.4f);
...
//MobileOption
    ...
    public void DeadDelay()
    {
        switch (deadCount)
        {
            ...
            case 1:
                ...
                VoiceLineManager.Instance.PlayRandomHansungMoan();
                break;
    ...
    /// <summary> 시간 제한을 업데이트 한다. </summary>
    void UpdateTimer()
    {
        float _duration = Duration * (GameConfig.IsPc() ? 1 : 2.0f);
        float time = (_duration - applyDuration);
        txt_timer.text = $"{(time / 60.0f) :f0} : {time%60.0f:f0}";

        Vector3 color1 = Vector3.zero;
        Vector3 color2 = Vector3.right;
        Vector3 result= Vector3.Lerp(color1, color2, applyDuration / _duration);

        txt_timer.color = new Color(result.x, result.y, result.z);
        //cs_timerLine.effectColor = 
    }
