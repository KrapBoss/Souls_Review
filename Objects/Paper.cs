...
public class Paper : GetObject
{
    ...
    public string nextLineTable = "Story";
    ...
    public virtual void FrozenOff()//그랩해제가 되지 않을 경우를 대비한 방법
    {
        ...
        if(nextLineTable.Contains("Voice") && applyIteratorNumber < iteratorNumber)
        {
            VoiceLineManager.Instance.EnterLine(new() { clip = nextLineKey, delay = 0, line = nextLineKey, reverb = false });
            VoiceLineManager.Instance.ShowLine(false);
        }
        else
        if (applyIteratorNumber<iteratorNumber && !string.IsNullOrEmpty(nextLineKey))
        {
            UI.staticUI.EnterLine(LocalLanguageSetting.Instance.GetLocalText(nextLineTable, nextLineKey), 6.0f);
            UI.staticUI.ShowLine();
        }

        if (!(nextLineTable.Contains("Voice") && applyIteratorNumber < iteratorNumber)) // 보이스가 아닌 경우에만 
        {
            // 후 하 사운드
