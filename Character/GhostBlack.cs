...
public class GhostBlack : MonoBehaviour
{
    ...
    public float shakeDistance = 5.0f;
    ...
    private void Awake()
    {
        ...
        _applyActiveCycle = 0;//Random.Range(ActiveCycle * 0.7f, ActiveCycle * 1.2f);
    ...
    private void Start()
    {
        player = PlayerEvent.instance.transform;
    }
    ...
    void Update()
    {
        ...
        if (activation)
        {
            if ((player.position - transform.position).magnitude < shakeDistance)
                EventManager.instance.CameraShake(1.0f, shakePower, true);
        }
    }
