...
public class Soul : MonoBehaviour, IMusicBox
{
    ...
    public Action act_afterAscention = null;
    ...
    private void Awake()
    {
        ...
        if (render)
        {
            ...
            block.SetColor("_BaseColor", SoulColor); // 원하는 색으로 변경
    ...
    public bool Ascention()
    {
        ...
        else
        {
            ...
            //8. 특정 동작을 수행
            if (act_afterAscention != null) act_afterAscention.Invoke();
    ...
    void Destroy()
    {
        ...
        if(GameManager.Instance.GetRemainSouls() > 0 && !GameManager.Instance.Tutorial)
        {
            string line = $"Soul_Line{Random.Range(1, 6)}";
            //string s = $"<color=#{ColorUtility.ToHtmlStringRGB(SoulColor)}>{LocalLanguageSetting.Instance.GetLocalText("Story", line) }</color>";
            VoiceLineManager.Instance.EnterLine( new() {clip = line, delay = 0, line = line, reverb = false }, nonTranslate : false);;
            VoiceLineManager.Instance.ShowLine(false);
        }
        //else if(GameManager.Instance.GetRemainSouls() == 0)
        //{
        //    VoiceLineManager.Instance.EnterLine(new() { clip = $"C_LastSoulsAscended", delay = 0, line = $"C_LastSoulsAscended", reverb = true });
        //}
