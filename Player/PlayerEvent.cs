...
public class PlayerEvent : MonoBehaviour
{
    ...
    //배터리 회복 메서드
    public bool BatteryRecovery()
    {
        //영구 손상 데미지가 회복치 보다 클 경우에만 사용 가능
        if (camPermanentDamage < DataSet.Instance.GameDifficulty.CameraRecovery) return false;
    ...
    public void CameraTakePicture()
    {
        ...
        // 튜토리얼 중이 아닐때 혹은 게임이 끝나지 않았을 때.
        if(!GameManager.Instance.Tutorial && !GameManager.Instance.GameEnded) camPermanentDamage += cam_TakePicture * cam_PermanentDamageRate;
