...
public class EventManager : MonoBehaviour
{
    ...
    CameraShaking _cameraShaking;                    //카메라 흔드는 효과
    CameraScreen _cameraScreen;                      //카메라에 시각적 효과를 적용
    ...
    #region 카메라
    //카메라 흔들기
    public void CameraShake(float t, float force, bool imediatly = false) { _cameraShaking.Shake(t, force, imediatly); }
    
    /// <summary>
    /// 카메라의 글리치 효과를 적용합니다
    /// </summary>
    /// <param name="on">ture : 켜기, false : 끄기</param>
    public void CameraGlitch(bool on)
    {
        if (!_cameraScreen) return;
        _cameraScreen.GlitchlSet(on);
    }
    #endregion
    ...
    public void ActiveGhostBlack()
    {
        ...
        if(!go_GhostBlack.activeSelf && DataSet.Instance.SettingValue.CurrentDiff != 0)
        {
            ...
            if(DataSet.Instance.SettingValue.CurrentDiff > 0)
            {
                ...
                VoiceLineManager.Instance.EnterLine(new() { clip = $"ActiveBlack1", delay = 0, line = $"ActiveBlack1", reverb = true });
                VoiceLineManager.Instance.EnterLine(new() { clip = $"ActiveBlack2", delay = 0, line = $"ActiveBlack2", reverb = true });
