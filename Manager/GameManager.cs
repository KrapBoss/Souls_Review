...
public class GameManager : MonoBehaviour
{
    ...
    public IntroBox cs_IntroBox;
    public Outro cs_Outro;
    public MapObjects cs_MapObject;
    ...
    {
        GameData.Init();
        GameData.introSkip = !tutorial;
        if (tutorial) TutorialStart();
        else SkipTutorial();
    }

    #region Intro && Tutorial
    public void TutorialStart() // 처음 보여주는 인트로
    {
        //Debug.Log("튜토리얼 시작");
    ...
    public void SkipTutorial()
    {
        ...
        PlayerEvent.instance.PlayerFaint(
            () => {
                ...
                VoiceLineManager.Instance.EnterLine(new() { clip = "H_TutorialPaint", delay = 0, line = "H_TutorialPaint", reverb = false });
        ...
        if(cs_IntroBox) cs_IntroBox.Skip();
    ...
    public void GameEnd()
    {
        ...
        //5분 이상의 기록이 존재할 경우
        if (DataSet.Instance.SettingValue.ClearTime[DataSet.Instance.SettingValue.CurrentDiff] > 120)
        {
            //현재 시간이 이전 기록보다 크다면?
...
public struct GameData
{
    ...
    /// <summary> 하양이가 첫 킬을 했다. </summary>
    public static bool firstKill;
    /// <summary> 인트로가 스킵이 되었다. </summary>
