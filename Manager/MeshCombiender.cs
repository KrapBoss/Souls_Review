...
public class MeshCombineder : MonoBehaviour
{
    ...
    public void CombineMeshesByKeyword(MeshFilter[] filters)
    {
        // 1. 머티리얼 키워드를 기준으로 병합할 메쉬들을 분류할 딕셔너리 할당
        Dictionary<string, List<CombineInstance>> keywordCombineMap = new Dictionary<string, List<CombineInstance>>();
        ...
        foreach (MeshFilter filter in filters)
        {
            ...
            // 배칭이 깨지는 주 원인인 URP 머티리얼의 활성화된 키워드 추출
            string keyword = renderer.sharedMaterial.shaderKeywords.Length > 0
                ? renderer.sharedMaterial.shaderKeywords[0]
                : "NonKeyword";

            if (!keywordCombineMap.ContainsKey(keyword))
                keywordCombineMap[keyword] = new List<CombineInstance>();

            // 2. 키워드별 그룹에 병합용 데이터(메쉬, 월드 좌표계 변환 데이터) 매핑
            CombineInstance ci = new CombineInstance();
            ci.mesh = filter.sharedMesh;
            ci.transform = filter.transform.localToWorldMatrix;

            keywordCombineMap[keyword].Add(ci);
        }

        // 3. 분류된 키워드 그룹별로 순회하며 완벽한 배칭(Batching)을 위한 단일 메쉬 생성
        foreach (var kvp in keywordCombineMap)
        {
            Mesh combinedMesh = new Mesh();

            // 모바일 기기 정점 제한(65,535개) 초과 방지를 위한 32비트 인덱스 포맷 지정
            combinedMesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32;

            // 동일한 키워드를 가진 메쉬들만 모아서 병합 실행 (드로우콜 대폭 감소)
